# Java Learning Journey

Repositorio dedicado ao estudo e pratica da linguagem Java, seguindo a trilha da Maratona Java (DevDojo). Aqui organizo meus exercicios por modulos logicos para demonstrar a evolucao do meu conhecimento.

---

## Mapa de Conhecimentos (Skill Map)

### 01. Sintaxe e Estruturas
Fundamentos da linguagem: tipos primitivos, operadores e controle de fluxo.
- Status: Em breve / Revisao

### 02. Orientacao a Objetos - Basico
Conceitos essenciais de classes, metodos e protecao de dados.
- Classes e Atributos: [Atividade 01](./02-Orientacao-a-Objetos-Basico/atividade-01), [Atividade 04](./02-Orientacao-a-Objetos-Basico/atividade-04)
- Encapsulamento (Private/Get/Set): [Atividade 02](./02-Orientacao-a-Objetos-Basico/atividade-02)
- Logica de Estado: [Atividade 08](./02-Orientacao-a-Objetos-Basico/atividade-08)

### 03. Orientacao a Objetos - Relacionamentos
Como os objetos interagem entre si.
- Associacao: [Atividade 05](./03-Orientacao-a-Objetos-Relacionamentos/atividade-05), [Atividade 06](./03-Orientacao-a-Objetos-Relacionamentos/atividade-06)
- Composicao/Agregacao: [Atividade 09](./03-Orientacao-a-Objetos-Relacionamentos/atividade-09)

### 04. Orientacao a Objetos - Avancado
Pilares avancados e abstracoes.
- Heranca: [Atividade 07](./04-Orientacao-a-Objetos-Avancado/atividade-07)
- Enums: [Atividade 03](./04-Orientacao-a-Objetos-Avancado/atividade-03)
- Interfaces: [Atividade 10](./04-Orientacao-a-Objetos-Avancado/atividade-10), [Atividade 12](./04-Orientacao-a-Objetos-Avancado/atividade-12)
- Polimorfismo: [Atividade 11](./04-Orientacao-a-Objetos-Avancado/atividade-11)

### 05. Excecoes
Tratamento de erros e robustez do codigo.
- Status: Proximo topico

### 06 a 13. Topicos Avancados
Estrutura preparada para os proximos passos da Maratona:
- 06. Classes Utilitarias (String, IO, NIO, Dates)
- 07. Colecoes
- 08. Generics
- 09. Programacao Funcional (Lambdas, Streams)
- 10. Threads e Concorrencia
- 11. Padroes de Projeto
- 12. JDBC
- 13. Testes Unitarios

### 14. Projetos
- [Projeto RPG](./14-Projetos/projeto-01): Aplicacao pratica integrando multiplos conceitos.

---
*Focado em me tornar um desenvolvedor Java de elite!*
